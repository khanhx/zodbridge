...
Initial release — a schema-first, decorator-free, Zod 4-driven mapping toolkit:
...
- **Serialize / deserialize**: JSON-safe codec (Date/BigInt/Map/Set),
  validate-then-transform, DoS bounds, sync + async.
- **Resolver graph** (`createResolver`): memoizing, cycle-safe, dependency-aware
  resolution over an injected adapter, with `strategies()` multi-source
  resolution (SKIP sentinel; `null` wins), `refresh`/`invalidate`, and
  schema-driven validated output.
